...
function checkAdminSession() {
    ...
    // অতিরিক্ত সুরক্ষা: লোকালস্টোরেজ বা সেশনে ইউজারটি আসলেই অ্যাডমিন কিনা তা যাচাই করা
    try {
        ...
        // যদি ইউজারটি সাধারণ ইউজার হয় কিন্তু অ্যাডমিন প্যানেলে ঢোকার চেষ্টা করে
...
async function fetchAdminProfileName() {
    ...
    try {
        ...
        if (!error && data) {
            // সিকিউরিটি চেক: ডাটাবেজেও ইউজারটি অ্যাডমিন কি না যাচাই করা
...
function filterAndRenderTables() {
    ...
    } else if (currentFilter === 'reset') {
        filtered = filtered.filter(u => u.resetRequested === true || u.passwordResetPending === true);
    }
...
function updateDashboardStats(data) {
    ...
    const resetRequests = data.filter(u => u.resetRequested === true || u.passwordResetPending === true).length;
    ...
    const resetElem = document.getElementById("statTotalVaults");
...
/* ==========================================================================
    Fast Password Reset Approval (Quick Password Update)
    ========================================================================== */
async function approveUser() {
    const userFieldInput = document.getElementById("appUser");
    const passFieldInput = document.getElementById("appPass");

    if (!userFieldInput || !passFieldInput) return;

    const queryVal = userFieldInput.value.trim();
    const newPassword = passFieldInput.value.trim();

    if (!queryVal || !newPassword) {
        showFlashPopup("Please enter both User ID/Email and the new password.", "error");
        return;
    }
    ...
    try {
        ...
        // Supabase-এর users টেবিলে userId, id অথবা email ফিল্ডের সাথে মিলিয়ে আপডেট করা হচ্ছে
        const { error } = await supabaseClient
            .from('users')
            .update({ 
            ...
            .or(`userId.eq.${queryVal},id.eq.${queryVal},email.eq.${queryVal}`);

        if (error) throw error;

        showFlashPopup("Password successfully updated and approved!", "success");
        userFieldInput.value = "";
        passFieldInput.value = "";
        fetchData();
    } catch (error) {
        console.error("Password reset error:", error);
        showFlashPopup("Failed to update password: " + error.message, "error");
    }
}

/* ==========================================================================
    Permanent Delete Account Function (অ্যাকাউন্ট চিরতরে মুছে ফেলা)
    ========================================================================== */
async function deleteUserAccount(userId, userName) {
    const confirmDelete = confirm(`Are you sure you want to PERMANENTLY delete the account for "${userName}" (ID: ${userId})? This action cannot be undone and they will have to create a new account to log in again.`);
    ...
    try {
        ...
        showFlashPopup(`Account for "${userName}" successfully deleted from database!`, 'success');
        fetchData(); 
    } catch (error) {
...
/* ==========================================================================
    Custom Flash Popup Notification System with Icons
    ========================================================================== */
